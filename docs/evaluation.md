...
The recorded support-v1 live run has ten latency samples (p50 2.13 s, p95 4.96 s), ten provider generation completions, eight schema/citation-valid answers, two validation failures, and two expected-abstention disagreements. Exact monetary cost is not reported. The support-v2 contract passed separate real activation checks; a complete v2 quality evaluation remains pending under the daily evaluation-session cap. Earlier results are preserved and do not become v2 scores. Reports carry Git SHA, dirty-tree flag, dataset/corpus hashes, model/manifest, prompt version, settings, timestamps, category counts and failures. Regenerate reports from a committed revision for final evidence. The UI never starts evaluations.
