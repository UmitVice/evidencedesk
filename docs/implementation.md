...
- **Development:** [web preview](https://evidencedesk-web-git-dev-umitvices-projects.vercel.app) pairs only with [dev API](https://evidencedesk-api-git-dev-umitvices-projects.vercel.app). Preview protection is retained. Hosted sessions are unavailable until the separate free dev database is created; dev never uses production credentials/state. Local native PostgreSQL 17.11 / pgvector 0.8.6 remains available.
- **Human action still needed for dev:** finish the prepared `evidencedesk-dev` project in the EvidenceDesk Free organization (`dzjfzmicvypgsxjmldlw`, Ohio), entering and saving its new database password. Then run `python3 scripts/enter-cloud-credentials.py --database-environment development --database-only`. Production credentials already work; no re-entry is needed. This is a new-credential browser handoff, not an exhausted-free-slot or billing limitation.
